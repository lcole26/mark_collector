...
export function handleInput(input) {
  ...
  if (command === enterKeyCommand && input.srcElement.name === inputName && (mark_collect_manager.current_data_stack && !mark_collect_manager.current_data_stack.length == 0)) {
    ...
    // let output = "";
    let analysis = processAnalysis([formatted_text]);
    // if (analysis) {
    ...
    afflicted_data.forEach(frequency => {
      let new_osc = new Wad({
        ...
        loop: true,
        panning: [random_in_range(-1, 1), random_in_range(-1, 1), random_in_range(-1, 1)],
        panningModel: 'HRTF',
    ...
    mark_collect_manager.osc_collector.push(iterator);
